...
class BaseProp(Generic[PR]):
    ...
    def set(self, **kwds: Any) -> PR:
        return self.ros.post_as(f"{self.filename}/set", None, kwds) # type: ignore
...
class BaseProps(Generic[PR]):
    ...
    def _getid(o: PR | str):
        if isinstance(o, str):
            return o
        assert hasattr(o, "id"), "Object must have an 'id' attribute"
        return getattr(o, "id")
    ...
    def unset(self, o: PR, keys: List[str]):
        assert self._write, "Not writeable"
        return self.ros.patch_as(
            f"{self.filename}/{self._getid(o)}",
            self.cl,
